...
1.  **Timezone**: All analysis and chart display are based on **UTC+1** (Central European Time).
    - _Note_: The "First 4H Candle" logic must be adjusted to match the user's local time perception if needed, or strictly follow NY time but displayed in UTC+1.
    - **Decision**: Strategy logic uses **UTC+1**. The "First 4H Candle" is the one starting at 00:00 UTC+1 (or user defined start).
2.  **The Range**:
    - Identify the **first 4-hour candle** of the trading day (e.g., 00:00 - 04:00 NY Time).
    - Mark its **High** and **Low**.
    - This creates a "Range Box" that extends to the end of the day (23:59 NY Time).
...
3.  **Risk Management**:
    ...
    - **Take Profit (TP)**: **Dynamic Structure-Based**.
      ...
      - **Constraint**: Minimum Reward-to-Risk (RR) of **1.5R**.
      - **Fallback**: If no structure found, use fixed **2R**.
    - **Position Sizing**: 0.5% Risk per Trade.

### 1.3. Performance Targets (Achieved V5)

- **Win Rate**: **72.22%** (Target 80% approached)
- **Avg RR**: **1.17R** (Profitable with 72% WR)
- **Max Drawdown**: **1.49%** (Target <3%)
