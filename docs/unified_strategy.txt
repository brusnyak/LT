...
from trading_bot.strategy.strategy_base import Strategy as StrategyBase
...
from trading_bot.analysis.session_analyzer import SessionAnalyzer
from trading_bot.utils.helpers import (
    standardize_timeframe, timeframe_to_minutes, compare_timeframes,
    get_higher_timeframe, get_lower_timeframe, is_in_kill_zone
)
...
class UnifiedStrategy(StrategyBase):
    ...
    def __init__(self, data_processor=None):
        ...
        super().__init__("Unified Strategy")
        ...
        from trading_bot.analysis.multi_timeframe_analyzer import MultiTimeframeAnalyzer
        ...
        self.sessions = {
            'london': (time(7, 0), time(16, 0)),     # London: 7:00-16:00 UTC
            'new_york': (time(12, 0), time(21, 0)),  # New York: 12:00-21:00 UTC
            'tokyo': (time(0, 0), time(9, 0)),       # Tokyo: 0:00-9:00 UTC
            'sydney': (time(22, 0), time(7, 0))      # Sydney: 22:00-7:00 UTC
        }
    ...
    def _generate_intraday_signals(self, df: pd.DataFrame, symbol: str, timeframe: str, 
        ...
        try:
            ...
            bullish_pattern = any(p['type'] in ['bullish_reversal', 'bullish_continuation'] for p in candle_patterns)
            bearish_pattern = any(p['type'] in ['bearish_reversal', 'bearish_continuation'] for p in candle_patterns)
            
            # Check if we're in a kill zone time
            in_kill_zone = False
            if isinstance(df.index, pd.DatetimeIndex):
                current_time = df.index[-1].to_pydatetime()
                in_kill_zone = self._is_in_kill_zone(current_time)
            ...
            if (market_bias == 'bullish' or market_bias == 'neutral') and (
                ...
                signal = {
                    ...
                    'indicators': {
                        ...
                        'pattern': [p['name'] for p in candle_patterns if p['type'] in ['bullish_reversal', 'bullish_continuation']],
            ...
            if (market_bias == 'bearish' or market_bias == 'neutral') and (
                ...
                signal = {
                    ...
                    'indicators': {
                        ...
                        'pattern': [p['name'] for p in candle_patterns if p['type'] in ['bearish_reversal', 'bearish_continuation']],
                        'in_kill_zone': in_kill_zone,
                        'volume_surge': volume_surge
                    }
                }
    ...
    async def generate_signals(self, symbol, timeframe, limit=100):
        ...
        try:
            ...
            if hasattr(self.data_processor, 'get_data_sync'):
                higher_tf_data = self.data_processor.get_data_sync(symbol, higher_tf, limit)
            else:
                # Fallback to get_historical_data but handle it properly
                try:
                    higher_tf_data = await self.data_processor.get_historical_data(symbol, higher_tf, limit)
                except TypeError:
                    # If it's not awaitable, try direct call
                    higher_tf_data = self.data_processor.get_historical_data(symbol, higher_tf, limit)
            ...
            if hasattr(self.data_processor, 'get_data_sync'):
                current_tf_data = self.data_processor.get_data_sync(symbol, timeframe, limit)
            else:
                try:
                    current_tf_data = await self.data_processor.get_historical_data(symbol, timeframe, limit)
                except TypeError:
                    current_tf_data = self.data_processor.get_historical_data(symbol, timeframe, limit)
            ...
            if hasattr(self.data_processor, 'get_data_sync'):
                lower_tf_data = self.data_processor.get_data_sync(symbol, lower_tf, limit*2)
            else:
                try:
                    lower_tf_data = await self.data_processor.get_historical_data(symbol, lower_tf, limit*2)
                except TypeError:
                    lower_tf_data = self.data_processor.get_historical_data(symbol, lower_tf, limit*2)
    ...
    def generate_signal(self, df: pd.DataFrame, symbol: str, timeframe: str, **kwargs) -> Dict:
        """
        ...
        try:
            ...
            # If no signals, return empty dict
            if not signals:
                return {}
    ...
    def analyze(self, df: pd.DataFrame, symbol: str, timeframe: str) -> Dict:
        ...
        try:
            ...
            # Run ICT analysis
            ict_analysis = self._analyze_with_ict(df, symbol, timeframe)
            
            # Run SMC analysis
            smc_analysis = self._analyze_with_smc(df, symbol, timeframe)
            ...
            # Perform multi-timeframe analysis if possible
            mtf_analysis = {}
            if hasattr(self, 'analyze_multi_timeframe'):
                mtf_analysis = self._perform_mtf_analysis(symbol, timeframe)
            ...
            return {
                ...
                'interest_zones': ict_analysis.get('interest_zones', []),
            ...
            logger.error(f"Error in unified strategy analysis: {e}", exc_info=True)
    ...
    def _determine_market_bias(self, analysis: Dict) -> str:
        """
    ...
    def _analyze_higher_timeframe(self, df: pd.DataFrame, symbol: str, timeframe: str) -> Dict:
        ...
        try:
            ...
            try:
                ict_analysis = self.ict_analyzer.analyze(df)
            except Exception as e:
            ...
            try:
                tech_analysis = self.technical_analyzer.analyze(df)
            except Exception as e:
    ...
    def _analyze_middle_timeframe(self, df: pd.DataFrame, symbol: str, timeframe: str, 
        ...
        try:
            ...
            try:
                if hasattr(self, 'session_analyzer'):
                    if hasattr(self.session_analyzer, 'analyze'):
                        session_analysis = self.session_analyzer.analyze(df)
                    elif hasattr(self.session_analyzer, 'analyze_sessions'):
                        session_analysis = self.session_analyzer.analyze_sessions(df)
                    else:
                        session_analysis = {}
                else:
                    session_analysis = {}
            ...
            result['liquidity_sweeps'] = ict_analysis.get('liquidity_sweeps', [])
            
            # Get session highs/lows
            result['session_highs_lows'] = session_analysis.get('session_levels', {})
            ...
            for fvg in ict_analysis.get('fair_value_gaps', []):
                ...
                (market_bias == 'bearish' and fvg.get('type') == 'bearish'):
                    interest_zones.append({
                        'type': 'fair_value_gap',
                        'price_range': fvg.get('price_range', [0, 0]),
            ...
            for bb in smc_analysis.get('breaker_blocks', []):
                ...
                (market_bias == 'bearish' and bb.get('type') == 'bearish'):
                    interest_zones.append({
                        'type': 'breaker_block',
                        'price_range': bb.get('price_range', [0, 0]),
            ...
            for sweep in result['liquidity_sweeps']:
                ...
                if sweep.get('candle_index', 0) >= len(df) - 20:
        ...
        try:
            ...
            if market_bias in ['bullish', 'bearish']:
                ...
                if liquidity_sweeps:
                    ...
                    recent_sweeps = sorted(liquidity_sweeps, key=lambda x: x.get('candle_index', 0), reverse=True)
                    
                    # Check if we have a recent sweep (within last 5 candles)
                    if recent_sweeps and recent_sweeps[0].get('candle_index', 0) >= len(df) - 5:
    ...
    def _analyze_with_ict(self, df: pd.DataFrame, symbol: str = None, timeframe: str = None) -> Dict:
        ...
        try:
            ...
            return self.ict_analyzer.analyze(df, symbol, timeframe)
        except Exception as e:
    ...
    def _analyze_with_smc(self, df: pd.DataFrame, symbol: str = None, timeframe: str = None) -> Dict:
        ...
        try:
            ...
            return self.smc_analyzer.analyze(df, symbol, timeframe)
        except Exception as e:
            logger.error(f"Error in SMC analysis: {e}", exc_info=True)
            return {
                'market_structure': 'neutral',
    ...
    def _calculate_risk_reward(self, entry_price: float, stop_loss: float, take_profit: float) -> float:
        ...
        from trading_bot.utils.helpers import calculate_risk_reward
        return calculate_risk_reward(entry_price, stop_loss, take_profit)
    ...
    def _calculate_position_size(self, account_size: float, risk_percentage: float, 
        ...
        """
        # Use the imported helper function directly
        from trading_bot.utils.helpers import calculate_position_size
        
        # Determine pair type based on symbol
        pair_type = self._get_market_type(symbol)
        
        return calculate_position_size(account_size, risk_percentage, entry_price, stop_loss, pair_type)
    ...
    def _is_trading_time(self) -> bool:
        ...
        """
        # Use the imported helper function directly
        from trading_bot.utils.helpers import is_trading_time
    ...
    def _calculate_session_range(self, df: pd.DataFrame, session: str) -> Dict:
        """
        Calculate price range for a specific trading session
        
        Args:
            df (pd.DataFrame): OHLCV data
            session (str): Trading session ('london', 'ny', 'asia', 'overlap')
            
        Returns:
            dict: Session high and low prices
        """
        try:
            ...
            session_candles = df[df.index.hour.isin(range(start_hour, end_hour + 1))]
            ...
            today_session = session_candles[session_candles.index.date == today]
            
            # If no data for today, use yesterday's session
            if today_session.empty:
                yesterday = today - pd.Timedelta(days=1)
                today_session = session_candles[session_candles.index.date == yesterday]
            
            # If still no data, use the most recent session
            if today_session.empty:
                # Get the most recent date in the data
                most_recent_date = df.index.date.max()
                today_session = session_candles[session_candles.index.date == most_recent_date]
            
            if today_session.empty:
                return {}
            
            # Calculate session high and low
            session_high = today_session['high'].max()
            session_low = today_session['low'].min()
            ...
            logger.error(f"Error calculating session range: {e}", exc_info=True)
            return {}
